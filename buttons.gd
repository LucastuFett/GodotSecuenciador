...
const launchColors = [Color8(255,50,37),Color8(0,134,12),Color8(63,119,255),Color8(212,133,28),Color8(241,84,174),Color8(11,171,201),Color8(245,95,76),Color8(206,1,250),
...
func _ready():
	...
	for btn in listbtn:
		...
		btn.button_up.connect(_buttonUp.bind(n))
		btn.text = OS.get_keycode_string(beatButtons[n])
		n += 1

func _input(event: InputEvent) -> void:
	if event is InputEventKey:
		for i in range(listbtn.size()):
			if event.keycode == beatButtons[i]:
				if event.pressed and not event.echo:
					# simulate button press
					listbtn[i].button_pressed = true
					buttonsPressed[i] = true
					listbtn[i].emit_signal("pressed")
				elif not event.pressed:
					# simulate button release
					listbtn[i].button_pressed = false
					buttonsPressed[i] = false
					listbtn[i].emit_signal("button_up")

# Called every frame. 'delta' is the elapsed time since the previous frame.
func _process(delta):
	pass

# Función para actualizar los colores de la botonera
func updateColors():
	match mainState:
		PROG:
			...
			#TODO: Si está en modo Play o Channel, utilizar los botones para prender o apagar canales, al apagar canal mandar un allnotesoff de ese canal
			for i in len(listbtn):
				var num = i
				if mode32 && half:
					num += 16
		...
		PLAY:
			for i in len(listbtn):
				listbtn[i].add_theme_stylebox_override("normal",greyStyle)
			for i in 16:
				if (channelEnabled[i] && channels[i]):
					listbtn[i].add_theme_stylebox_override("normal",blueStyle)
		LAUNCH:
			for i in len(listbtn):
				listbtn[i].add_theme_stylebox_override("pressed",greyStyle)
				listbtn[i].add_theme_stylebox_override("normal",greyStyle)
			for i in 16:
				if (buttonsPressed[i] || listbtn[i].button_pressed):
							...
							if hold == 0:
								for j in 10:
									if offMessages[j*32 + num][0] == 0:
										offMessages[(j*32) + num] = [0x90 | channel,midiNote,0]
										break
							elif hold == 1:
								hold = 2
							elif hold == 2:
...
# Función que se llama cuando se importa un nuevo archivo, actualizando la estructura beatsPerTone y channels
